...
// AuthRes A server responds to an AuthReq message with an
// AuthRes message over the control channel.
//
...
// ReqTunnel A client sends this message to the server over the control channel
...
// NewTunnel When the server opens a new tunnel on behalf of
...
// ReqProxy When the server wants to initiate a new tunneled connection, it sends
...
// RegProxy After a client receives a ReqProxy message, it opens a new
// connection to the server and sends a RegProxy message.
type RegProxy struct {
	ClientId string
}

// StartProxy This message is sent by the server to the client over a *proxy* connection before it
// begins to send the bytes of the proxied request.
type StartProxy struct {
	Url        string //URL of the tunnel this connection is being proxied for
	ClientAddr string //Network address of the client initiating the connection to the tunnel
}

// Ping A client or server may send this message periodically over
...
// Pong Sent by a client or server over the control channel to indicate
// it received a Ping.
type Pong struct {
}
