...
func (h *Http) readRequests(tee conn.Tee, lastTxn chan *HttpTxn, connCtx interface{}) {
	...
	for {
		...
		// make sure we read the body of the request so that we don't block the writer
...
func (h *Http) readResponses(tee conn.Tee, lastTxn chan *HttpTxn) {
	for txn := range lastTxn {
		res, err := http.ReadResponse(tee.ReadBuffer(), txn.Req.Request)
		...
		// make sure we read the body of the response so that we don't block the reader
		_, _ = httputil.DumpResponse(res, true)
		txn.Res = &HttpResponse{Response: res}
		...
		if txn.Req.Header.Get("Upgrade") == "websocket" {
			...
			wg.Add(2)
			go func() {
				_, _ = io.ReadAll(tee.WriteBuffer())
				wg.Done()
			}()
			go func() {
				_, _ = io.ReadAll(tee.ReadBuffer())
				wg.Done()
			}()
			wg.Wait()
			break
...
// delegateReader is a reader that delegates to another reader,once it arrivas on a channel
...
func DumpRequestOut(req *http.Request, body bool) ([]byte, error) {
	...
	go func() {
		_req, _ := http.ReadRequest(bufio.NewReader(pr))
		// THIS IS THE PART THAT'S BROKEN IN THE STDLIB (as of Go 1.3)
		if _req != nil && req.Body != nil {
