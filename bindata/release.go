...
func writeReleaseHeader(w io.Writer, c *Config) error {
	...
	if c.NoCompress {
		if c.NoMemCopy {
			err = headerUncompressedNomemcopy(w, c)
		} else {
			err = headerUncompressedMemcopy(w, c)
		}
	} else {
		if c.NoMemCopy {
			err = headerCompressedNomemcopy(w, c)
		} else {
			err = headerCompressedMemcopy(w, c)
		}
	}
	if err != nil {
		return err
...
func headerCompressedNomemcopy(w io.Writer, c *Config) error {
	var header string
...
func headerCompressedMemcopy(w io.Writer, c *Config) error {
	var header string
...
func headerUncompressedNomemcopy(w io.Writer, c *Config) error {
	var header string
...
func headerUncompressedMemcopy(w io.Writer, c *Config) error {
	var header string
