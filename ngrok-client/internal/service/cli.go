...
Commands:
	...
	ngrok list				  List tunel names from config file
	ngrok help				  Print help
	ngrok version			  Print ngrok version

Examples:
	ngrok start www api blog pubsub
	ngrok -log=stdout -confg=ngrok.yml start ssh
	ngrok start-all
	ngrok version
`
...
func ParseArgs() (opts *Options, err error) {
	flag.Usage = func() {
		_, _ = fmt.Fprintf(os.Stderr, usage1, os.Args[0])
		flag.PrintDefaults()
		_, _ = fmt.Fprintf(os.Stderr, usage2)
	}
	...
	autoToken := flag.String(
		"auth-token",
		"",
		"Authentication token for identifying an ngrok-plus.com account")
	...
	case "help":
		...
		os.Exit(0)
	case "":
		err = fmt.Errorf("error: Specify a local port to tunnel to ,or an ngrok command.\n\nExample: To expose port 80, run 'ngrok 80'")
		return
	default:
		if len(flag.Args()) > 1 {
			err = fmt.Errorf("you may only specify one port to tunnel to on the command line, got %d: %v", len(flag.Args()), flag.Args())
			return
		}
