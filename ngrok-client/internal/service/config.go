...
func LoadConfiguration(opts *Options) (config *Configuration, err error) {
	...
	var configBuf []byte
	if configBuf, err = os.ReadFile(configPath); err != nil {
		// failure to read a configuration file is only a fatal error if the user specified one explicitly
		if opts.config != "" {
	...
	// set configuation defaults
	if config.ServerAddr == "" {
		config.ServerAddr = consts.DefaultServerAddr
	}

	if config.InspectAddr == "" {
		config.InspectAddr = consts.DefaultInspectAddr
	}

	if config.HttpProxy == "" {
		config.HttpProxy = os.Getenv(consts.HttpProxy)
	}
	...
	for name, t := range config.Tunnels {
		...
		//use the name of the tunnel as the subdomain if none is spcified
		if t.Hostname == "" && t.Subdomain == "" {
			// XXX: a crude heuristic,really we should be checking if the last part is a TLD
	...
	case "start":
		...
		requestTunnels := make(map[string]bool)
		for _, arg := range opts.args {
			requestTunnels[arg] = true
...
func defaultPath() string {
	...
	homeDir := os.Getenv(consts.Home)
	if _err != nil {
		_ = log.Warn("Failed to get user's home directory: %s. Using $HOME: %s", _err.Error(), homeDir)
	} else {
...
func normalizeAddress(addr, propName string) (string, error) {
...
func SaveAuthToken(configPath, authToken string) (err error) {
	...
	if newConfigBytes, err = yaml.Marshal(c); err != nil {
		return
	}
