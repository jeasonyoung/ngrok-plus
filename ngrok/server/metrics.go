...
func NewKeenIoMetrics(batchInterval time.Duration) *KeenIoMetrics {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				_ = k.Error("KeenIoMetrics failed: %v", r)
			}
		}()
		...
		for {
			...
			case <-batchTimer:
				...
				if err != nil {
					_ = k.Error("Failed to serialize metrics payload: %v, %v", batch, err)
				} else {
					for key, val := range batch {
						k.Debug("Reporting %d metrics for %s", len(val), key)
					}

					_, _ = k.AuthedRequest("POST", "/events", bytes.NewReader(payload))
				}
				batch = make(map[string][]interface{})
			}
		}
...
func (k *KeenIoMetrics) AuthedRequest(method, path string, body *bytes.Reader) (resp *http.Response, err error) {
	...
	if err != nil {
		_ = k.Error("Failed to send metric event to keen.io %v", err)
	} else {
		...
		if resp.StatusCode != 200 {
			...
			_ = k.Error("Got %v response from keen.io: %s", resp.StatusCode, _bytes)
		}
	}
