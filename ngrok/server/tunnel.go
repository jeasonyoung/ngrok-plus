...
func (t *Tunnel) listenTcp(listener *net.TCPListener) {
	for {
		defer func() {
			if r := recover(); r != nil {
				_ = log.Warn("listenTcp failed with error %v", r)
			}
		}()
		...
		if err != nil {
			...
			_ = t.Error("Failed to accept new TCP connection: %v", err)
			continue
		}
...
func (t *Tunnel) HandlePublicConnection(publicConn conn.Conn) {
	...
	defer func() {
		if r := recover(); r != nil {
			_ = publicConn.Warn("HandlePublicConnection failed with error %v", r)
		}
	}()
	...
	_ = util.PanicToError(func() { t.ctl.out <- &msg.ReqProxy{} })
