...
func autoUpdate(s mvc.State, token string) {
	...
	if err != nil {
		_ = log.Error("Failed to create update with signature: %v", err)
		return
	}

	_update := func() (tryAgain bool) {
		...
		if errors.Is(err, check.NoUpdateAvailable) {
			log.Info("No update available")
			return true
		} else if err != nil {
			_ = log.Error("Error while checking for update: %v", err)
			return true
		}

		if result.Initiative == check.INITIATIVE_AUTO {
			if err := up.CanUpdate(); err != nil {
				_ = log.Error("Can't update: insufficient permissions: %v", err)
...
func applyUpdate(s mvc.State, result *check.Result) {
	...
	_ = log.Error("Error while updating ngrok: %v", err)
	if errRecover != nil {
		_ = log.Error("Error while recovering from failed ngrok update, your binary may be missing: %v", errRecover.Error())
	}
