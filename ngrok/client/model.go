...
import (
	...
	"ngrok-plus/ngrok/msg"
	"ngrok-plus/ngrok/proto"
	"ngrok-plus/ngrok/util"
	"ngrok-plus/ngrok/version"
	"runtime"
	"strings"
	"sync/atomic"
	"time"
...
func (c *ClientModel) PlayRequest(tunnel mvc.Tunnel, payload []byte) {
	...
	if err != nil {
		_ = c.Warn("Failed to open private leg to %s: %v", tunnel.LocalAddr, err)
		return
	}
...
func (c *ClientModel) control() {
	...
	for _, config := range c.tunnelConfig {
		...
		for _proto, _ := range config.Protocols {
			protocols = append(protocols, _proto)
		}
...
func (c *ClientModel) proxy() {
	...
	if err != nil {
		_ = log.Error("Failed to establish proxy connection: %v", err)
		return
	}
	defer func(co conn.Conn) {
		_ = co.Close()
	...
	if err != nil {
		_ = remoteConn.Error("Failed to write RegProxy: %v", err)
		return
	}
	...
	if err = msg.ReadMsgInto(remoteConn, &startPxy); err != nil {
		_ = remoteConn.Error("Server failed to write StartProxy: %v", err)
		return
	}

	tunnel, ok := c.tunnels[startPxy.Url]
	if !ok {
		_ = remoteConn.Error("Couldn't find tunnel for proxy: %s", startPxy.Url)
		return
	}
	...
	localConn, err := conn.Dial(tunnel.LocalAddr, "prv", nil)
	if err != nil {
		_ = remoteConn.Warn("Failed to open private leg %s: %v", tunnel.LocalAddr, err)
		...
		if tunnel.Protocol.GetName() == "http" {
			...
			_, _ = remoteConn.Write([]byte(fmt.Sprintf(`HTTP/1.0 502 Bad Gateway
