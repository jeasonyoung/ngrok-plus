...
func (whv *WebHttpView) updateHttp() {
	...
	for txn := range txnUpdates {
		...
		if htxn.UserCtx == nil {
			...
			if err != nil {
				_ = whv.Error("Failed to dump request: %v", err)
				continue
			}
			...
			if err != nil {
				_ = whv.Error("Failed to dump response: %v", err)
				continue
			}
			...
			if err != nil {
				_ = whv.Error("Failed to serialized txn payload for websocket: %v", err)
			}
...
func (whv *WebHttpView) register() {
	http.HandleFunc("/http/in/replay", func(w http.ResponseWriter, r *http.Request) {
		defer func() {
			if r := recover(); r != nil {
				...
				_ = whv.Error("Replay failed: %v", err)
		...
		if txn, ok := whv.idToTxn[txnid]; ok {
			...
			_, _ = w.Write([]byte(http.StatusText(200)))
		} else {
	...
	http.HandleFunc("/http/in", func(w http.ResponseWriter, r *http.Request) {
		defer func() {
			if r := recover(); r != nil {
				err := util.MakePanicTrace(r)
				_ = whv.Error("HTTP web view failed: %v", err)
				http.Error(w, err, 500)
			}
		}()
